...
import { Building2, Loader2, ArrowRight, MapPin, DollarSign, Ruler, Bed, Bath, Car, Building, AlertTriangle } from 'lucide-react';
import { updateAgencyProperty, fetchAgencyAgents } from '../../api/agency';
import type { AgencyAgent } from '../../api/agency';
import client from '../../api/client';

interface PropertyType { id: number; name_ar: string; name_en: string; }
...
export default function PropertyEdit() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      client.get(`/agency/properties/${pid}`).then(r => {
        const p = r.data.data;
        setForm({
          ...
          governorate_id: String(p.governorate_id || ''),
        ...
        if (p.governorate_id) {
          client.get(`/locations/${p.governorate_id}/areas`).then(r => {
            const d = r.data.data ?? r.data;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateAgencyProperty(parseInt(id), payload);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-stone-900">{L('تعديل العقار', 'Edit Property')}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="card-3d p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" required value={form.title_ar} onChange={e => handleChange('title_ar', e.target.value)} className={inputCls('title_ar')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block">{L('عنوان العقار (إنجليزي)', 'Title (English)')}</label>
              <input type="text" required value={form.title_en} onChange={e => handleChange('title_en', e.target.value)} className={inputCls('title_en')} />
            </div>
            <div>
              ...
              <div className="flex gap-2">
                <input type="number" required value={form.price} onChange={e => handleChange('price', e.target.value)} className={`${inputCls('price')} flex-1`} />
        ...
        <section className="card-3d p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={form.address_ar} onChange={e => handleChange('address_ar', e.target.value)} className={inputCls('address_ar')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block">{L('العنوان (إنجليزي)', 'Address (English)')}</label>
              <input type="text" value={form.address_en} onChange={e => handleChange('address_en', e.target.value)} className={inputCls('address_en')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block">Latitude</label>
              <input type="text" value={form.lat} onChange={e => handleChange('lat', e.target.value)} className={inputCls('lat')} dir="ltr" />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block">Longitude</label>
              <input type="text" value={form.lng} onChange={e => handleChange('lng', e.target.value)} className={inputCls('lng')} dir="ltr" />
            </div>
          </div>
        </section>
        ...
        <section className="card-3d p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block"><Ruler className="w-3.5 h-3.5 inline mr-1" /> {L('المساحة (م²)', 'Area (m²)')}</label>
            ...
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block"><Bed className="w-3.5 h-3.5 inline mr-1" /> {L('غرف النوم', 'Bedrooms')}</label>
              <input type="number" value={form.bedrooms} onChange={e => handleChange('bedrooms', e.target.value)} className={inputCls('bedrooms')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block"><Bath className="w-3.5 h-3.5 inline mr-1" /> {L('الحمامات', 'Bathrooms')}</label>
              <input type="number" value={form.bathrooms} onChange={e => handleChange('bathrooms', e.target.value)} className={inputCls('bathrooms')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block"><Car className="w-3.5 h-3.5 inline mr-1" /> {L('مواقف سيارات', 'Parking')}</label>
        ...
        <section className="card-3d p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea rows={4} required value={form.description_ar} onChange={e => handleChange('description_ar', e.target.value)} className={inputCls('description_ar')} />
            </div>
            <div>
              <label className="text-sm font-medium text-stone-700 mb-1.5 block">{L('الوصف (إنجليزي)', 'Description (English)')}</label>
              <textarea rows={4} required value={form.description_en} onChange={e => handleChange('description_en', e.target.value)} className={inputCls('description_en')} />
            </div>
          </div>
        </section>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <select value={form.status} onChange={e => handleChange('status', e.target.value)} className="input-field w-40">
                ...
                <option value="reserved">{L('محجوز', 'Reserved')}</option>
                <option value="sold">{L('تم البيع', 'Sold')}</option>
                <option value="rented">{L('تم التأجير', 'Rented')}</option>
              </select>
            </div>
