...
@php use App\Models\CompanyDocument; @endphp
...
<div class="space-y-5">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight" style="color:var(--text-primary);">مستندات الشركات</h1>
            <p class="text-sm mt-0.5" style="color:var(--text-muted);">مراجعة واعتماد مستندات تفعيل الشركات — اعرف عميلك (KYC)</p>
        </div>
        <a href="{{ route('admin.companies.index') }}" class="btn btn-secondary btn-sm shrink-0" aria-label="العودة لقائمة الشركات">
            <x-heroicon name="apartment" class="text-sm" aria-hidden="true" />
            كل الشركات
        </a>
    </div>
    ...
    @include('admin.partials._filter_bar', [
        'route'              => route('admin.companies.documents'),
        'searchValue'        => request('search'),
        'searchPlaceholder'  => 'اسم الشركة، الكود، أو رقم المستند…',
        ...
        'filters'            => [
            ...
            ['name'=>'document_type','label'=>'نوع المستند','selected'=>request('document_type'),'options'=>CompanyDocument::TYPES],
        ],
    ])
            ...
            <div class="table-container" style="border:none;border-radius:0;box-shadow:none;">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('admin.companies.show', $doc->company) }}" class="block font-bold" style="color:var(--text-primary);">{{ $doc->company->name }}</a>
                                <span class="font-mono text-xs" style="color:var(--text-muted);">{{ $doc->company->company_code }}</span>
                            </td>
                            <td>
...
<x-heroicon name="event_available" style="font-size:0.8rem;" aria-hidden="true" x-show="!$expired" />
                            ...
                            <td>
                                <div class="row-actions justify-center">
                                    ...
                                    <form method="POST" action="{{ route('admin.companies.documents.approve', $doc) }}"
                                          onsubmit="return confirm('اعتماد مستند « {{ $doc->type_label }} » للشركة « {{ $doc->company->name }} »؟')" class="inline">
                                        @csrf
                                    ...
                                    <button type="button" class="row-action is-danger" title="رفض" aria-label="رفض المستند"
                                            @click="$dispatch('open-doc-reject', {
                                                rejectUrl: @js(route('admin.companies.documents.reject', $doc)),
                                                merchantName: @js($doc->company->name),
                                                docType: @js($doc->type_label)
                                            })">
                                        <x-heroicon name="close" aria-hidden="true" />
                                    </button>
                                    @endif
                                    <a href="{{ route('admin.companies.documents.show', $doc->company) }}" class="row-action" title="كل مستندات الشركة" aria-label="كل مستندات الشركة">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="table-empty">
                                    ...
                                    <p class="text-sm mt-1">ستظهر هنا المستندات التي ترفعها الشركات لمراجعتها</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
...
<div x-data="docRejectModal()"
    ...
    <div class="modal modal-danger" @click.outside="close()" role="dialog" aria-modal="true" aria-labelledby="co-dr-title">
        <div class="modal-header">
            <div>
                <h3 class="modal-title font-extrabold" id="co-dr-title">رفض المستند</h3>
        ...
        <form method="POST" :action="rejectUrl">
            ...
            <div class="modal-body">
                <label for="co-dr-reason" class="label label-required">سبب الرفض</label>
                <textarea id="co-dr-reason" name="rejection_reason" rows="3" class="input" required minlength="3"
