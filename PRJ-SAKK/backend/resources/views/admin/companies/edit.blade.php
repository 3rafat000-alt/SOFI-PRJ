...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.companies.show', $company) }}" class="btn btn-sm btn-sukk-icon">
            <x-heroicon name="arrow_forward" />
        </a>
        <div>
            <h1 class="text-2xl font-extrabold" style="color: var(--text-primary)">تعديل الشركة</h1>
            <p class="text-sm mt-0.5" style="color: var(--text-muted)">{{ $company->company_code }} — {{ $company->name }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.companies.update', $company) }}" class="space-y-6">
            ...
            <div class="card-body">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="owner_name" value="{{ old('owner_name', $company->owner_name) }}" class="input">
                    </div>
                    <div>
                        <label class="label">البريد</label>
                        <input type="email" name="email" value="{{ old('email', $company->email) }}" class="input" dir="ltr">
                    </div>
                    <div>
                        <label class="label">الهاتف</label>
                        <input type="text" name="phone" value="{{ old('phone', $company->phone) }}" class="input" dir="ltr">
                    </div>
                    <div>
                        <label class="label">الرقم الضريبي</label>
                        <input type="text" name="tax_id" value="{{ old('tax_id', $company->tax_id) }}" class="input">
                    </div>
                    <div>
                        <label class="label">السجل التجاري</label>
                        <input type="text" name="commercial_register" value="{{ old('commercial_register', $company->commercial_register) }}" class="input">
                    </div>
                    <div>
        ...
        <div class="card">
            ...
            <div class="card-body space-y-6">
                <div class="flex items-center gap-6">
                    ...
                    <label class="flex items-center gap-3 text-sm font-bold" style="color: var(--text-primary)">
                        <input type="checkbox" name="payroll_enabled" value="1" @checked($company->payroll_enabled)
                               class="w-4 h-4 rounded" style="border-color:var(--border);color:var(--primary);accent-color:var(--primary);">
                        توزيع الرواتب مفعّل
                    </label>
                </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.companies.show', $company) }}" class="btn btn-secondary">إلغاء</a>
