...
@section('title', 'مستندات ' . $agent->name)

@section('breadcrumbs')
<a href="{{ route('admin.agents.index') }}" class="breadcrumb-item">الوكلاء</a>
<a href="{{ route('admin.agents.documents') }}" class="breadcrumb-item">المستندات</a>
...
<div class="space-y-5">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.agents.show', $agent) }}" class="btn btn-sukk-icon" aria-label="رجوع للوكيل">
                <x-heroicon name="arrow_forward" aria-hidden="true" />
            </a>
            <div>
                <h1 class="text-2xl font-extrabold tracking-tight" style="color:var(--text-primary);">{{ $agent->name }}</h1>
                <p class="text-sm mt-0.5" style="color:var(--text-muted);">مستندات التفعيل — {{ $agent->agent_code }}</p>
            </div>
        </div>
        <span class="badge badge-{{ $agent->kyc_status_color }}" style="font-size:0.8rem;padding:var(--space-xs) var(--space-md);">
    ...
    <div class="card" style="border-color:var(--danger);background:var(--danger-light);">
        <div class="card-body flex items-start gap-2" style="color:var(--danger);">
            ...
            <span class="text-sm font-bold">{{ $agent->kyc_rejection_reason }}</span>
        </div>
    </div>
    @endif
    ...
    @forelse($documents as $doc)
        @include('admin.agents.partials._document_viewer', [
            'doc'    => $doc,
            'entity' => $agent,
        ])
    @empty
        <div class="card">
            <div class="card-body">
                <div class="table-empty">
                    ...
                    <p class="text-sm mt-1">لم يرفع هذا الوكيل أي مستندات بعد</p>
                </div>
            </div>
        </div>
    @endforelse
...
<div x-data="docRejectModal()"
    ...
    <div class="modal modal-danger" @click.outside="close()" role="dialog" aria-modal="true" aria-labelledby="ags-dr-title">
        <div class="modal-header">
            <div>
                <h3 class="modal-title font-extrabold" id="ags-dr-title">رفض المستند</h3>
        ...
        <form method="POST" :action="rejectUrl">
            ...
            <div class="modal-body">
                <label for="ags-dr-reason" class="label label-required">سبب الرفض</label>
                <textarea id="ags-dr-reason" name="rejection_reason" rows="3" class="input" required minlength="3"
